...
export const createTeacher = createAsyncThunk("createTeacher", async (formData, { getState, rejectWithValue }) => {
  const state = getState()
  const CSRFToken = state.auth.CSRFToken
    try {
...
export const UpdateTeacher = createAsyncThunk("UpdateTeacher", async (formData, { getState, rejectWithValue }) => {
  const state = getState()
  const CSRFToken = state.auth.CSRFToken
    try {
      const { data } = await axios.post(
