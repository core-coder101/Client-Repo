...
export default function ChattingTesting() {
  ...
  useEffect(() => {
    ...
    function setActiveChat(f) {
      ...
      const newChatPerson = chat.container.querySelector(
        `[data-chat="${f.getAttribute("data-chat")}"]`
      );
  ...
  const GetStoredMessages = async (ID) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_HOST}api/MessageStoredData`,
        {
          receiver_id: ID,
        },
        {
  ...
  const GetEachStoredMessages = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_HOST}api/GetEachStoredMessages`,
        {
          headers: {
            "X-CSRF-TOKEN": CSRFToken,
            "Content-Type": "application/json",
  ...
  const Submit = async (e) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_HOST}api/PrivateMessage`,
        {
          message: message,
          receiver_id: ID,
        },
                  ...
                  <li
                    ...
                    onClick={() => {
                      ...
                      setUnreadMessage((prevMessages) =>
                        prevMessages.filter(
                          (message) => message.Sending_id !== user.id
                        )
                      );
                    }}
